...
def process_individual_sv(sv, hapmap_ref_file, referece_sequence, output_bam_folder, output_bam, sv_discovery_range, smoothing, test_function):
...
def calculate_methylation_diff_region_bam(sv_vcf, input_bam, reference_genome, 
    ...
    args_list = [(test_sv, hapmap_ref_file, referece_sequence, output_bam_folder, output_bam, sv_discovery_range, smoothing, test_function) 
...
def process_individual_sv_benchmark(sv, hapmap_ref_file, referece_sequence, output_bam_folder, output_bam, sv_discovery_range, benchmark_second_bam, smoothing, test_function):
    modification_dict = get_base_modification_dictionary_new_bam(bam_file=hapmap_ref_file, 
                                                             ref_seq=referece_sequence, 
                                                             chromosome=sv.chr, 
                                                             phase_region=(int(sv.ref_start)-sv_discovery_range, int(sv.ref_end)+sv_discovery_range), 
                                                             sv_supporting_reads=sv.supporting_reads, sv_id=sv.id, output_bam_folder=output_bam_folder, output_bam=output_bam)
    modification_dict = src.smoothing.smoothing_comp(modification_dict, key_diff_threshold=smoothing)
    benchmark_second_bam_sv_methylation_dict = get_base_modification_dictionary_new_bam(bam_file=benchmark_second_bam, 
                                                             ref_seq=referece_sequence, 
                                                             chromosome=sv.chr, 
                                                             phase_region=(int(sv.ref_start)-sv_discovery_range, int(sv.ref_end)+sv_discovery_range), 
                                                             sv_supporting_reads=None, sv_id=sv.id, output_bam_folder=output_bam_folder, output_bam=output_bam)
    ...
    if test_function == 'ttest':
        ranksum_dict = calculate_ttest(modification_dict, benchmark_second_bam_sv_methylation_dict)
    elif test_function == 'ranksum':
        ranksum_dict = calculate_ranksum(modification_dict, benchmark_second_bam_sv_methylation_dict)
    elif test_function == 'fisher':
        ranksum_dict = calculate_fisher(modification_dict, benchmark_second_bam_sv_methylation_dict)
    else:
        ranksum_dict = calculate_ttest(modification_dict, benchmark_second_bam_sv_methylation_dict)
...
def calculate_methylation_diff_region_benchmark(input_bam, input_vcf, reference_genome, output, sv_discovery_interval, benchmark_second_bam, min_supporting_reads, test_function, smoothing, threads):
    ...
    args_list = [(test_sv, input_bam, reference_genome, output, benchmark_second_bam, sv_discovery_interval, smoothing, test_function) 
