...
def _select_groups(discovered: list[SplitGroup], group_text: str | None) -> list[str]:
    ...
    if not group_text:
        if len(discovered) != 2:
            raise ValueError(
                "postprocess v1 requires exactly two split groups unless --groups is used"
            )
...
def _build_context(args) -> RunContext:
    ...
    run_id = args.run_id or f"postprocess_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    run_root = split_dir / "postprocess" / run_id
    tool_paths = {
    ...
    params = {
        ...
        "kanpig_threads": args.kanpig_threads,
        ...
        "modkit_threads": args.modkit_threads,
    }
    return RunContext(
        sample_id=sample_id,
...
def _write_run_manifest(ctx: RunContext) -> None:
    ...
    _write_json(ctx.manifest_dir / "postprocess_run_manifest.json", payload)


def _write_task_manifest(ctx: RunContext, rows: list[dict[str, str]]) -> None:
    path = ctx.manifest_dir / "postprocess_task_manifest.tsv"
    path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = [
        "task_id",
        "stage",
...
def _update_status(ctx: RunContext, task_id: str, payload: dict[str, object]) -> None:
    status_path = ctx.status_dir / "postprocess_status.json"
...
def _run_sniffles(ctx: RunContext, group_name: str) -> None:
    ...
    cmd = [
        ...
        str(output_snf),
        "--threads",
        str(ctx.params["sniffles_threads"]),
        "--mosaic",
...
def _run_collapse(ctx: RunContext) -> None:
    ...
    output_vcf = stage_dir / "collapsed.vcf.gz"
    ...
    _run_task(ctx=ctx, stage="collapse_inputs", command=merge_cmd, outputs=[merged_input])
    if not ctx.dry_run:
        subprocess.run(
    ...
    if not ctx.dry_run:
        subprocess.run(
            [ctx.tool_paths["bcftools"], "view", "-Oz", "-o", str(output_vcf), str(raw_output)],
            check=True,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
...
def _run_medaka(ctx: RunContext, group_name: str) -> None:
    ...
    _run_task(
        ...
        group_name=group_name,
        command=cmd,
        outputs=[stage_dir / "medaka_to_ref.TR.vcf"],
    )
...
def _run_tdb_merge(ctx: RunContext) -> None:
    ...
    cmd = [
        ...
        str(output_tdb),
        "--threads",
        str(ctx.params["tdb_merge_threads"]),
        str(input_a),
        str(input_b),
    ]
...
def _run_post_tdb(ctx: RunContext) -> None:
    script = ctx.params["post_tdb_script"]
    if not script:
        return
    merged_tdb = _merged_tdb_output_path(ctx)
    marker = ctx.run_root / "medaka_tandem" / "tdb" / "post_tdb.done"
    cmd = [
        str(script),
        "--merged-tdb",
        str(merged_tdb),
        ...
        ctx.sample_id,
        "--group-a",
        ctx.selected_groups[0],
        "--group-b",
        ctx.selected_groups[1],
        "--group-a-tdb",
        str(_tdb_output_path(ctx, ctx.selected_groups[0])),
        "--group-b-tdb",
        str(_tdb_output_path(ctx, ctx.selected_groups[1])),
        "--output-root",
        str(ctx.run_root / "medaka_tandem" / "post_tdb"),
    ]
    extra = str(ctx.params["post_tdb_args"]).strip()
    if extra:
        cmd.extend(shlex.split(extra))
    command_path, stdout_path, stderr_path = _task_paths(ctx, "post_tdb", None)
    _write_command(command_path, cmd)
    if _should_skip(ctx=ctx, stage="post_tdb", outputs=[marker], group_name=None):
        _update_status(ctx, "post_tdb", {"state": "skipped", "updated_at": _now_utc(), "outputs": [str(marker)]})
        return
    _update_status(ctx, "post_tdb", {"state": "running", "started_at": _now_utc(), "outputs": [str(marker)]})
    try:
        _run_command(cmd=cmd, stdout_path=stdout_path, stderr_path=stderr_path, dry_run=ctx.dry_run, shell=False)
        if not ctx.dry_run:
            marker.parent.mkdir(parents=True, exist_ok=True)
            marker.write_text("ok\n")
            _record_done(ctx=ctx, stage="post_tdb", outputs=[marker], command=cmd, group_name=None)
            _update_status(ctx, "post_tdb", {"state": "completed", "finished_at": _now_utc(), "outputs": [str(marker)]})
        else:
            _update_status(ctx, "post_tdb", {"state": "dry_run", "finished_at": _now_utc(), "outputs": [str(marker)]})
    except Exception as exc:
        _record_failure(ctx=ctx, stage="post_tdb", command=cmd, error=exc, group_name=None)
        _update_status(ctx, "post_tdb", {"state": "failed", "finished_at": _now_utc(), "error": str(exc)})
        raise
...
def _python_entrypoint() -> list[str]:
    return [sys.executable, "-m", "sniffcell.main", "postprocess"]


def _build_recursive_cli(ctx: RunContext, stage_list: str, groups: str | None = None) -> list[str]:
    ...
    if ctx.params["post_tdb_script"]:
        cmd.extend(["--post-tdb-script", str(ctx.params["post_tdb_script"])])
    if ctx.params["post_tdb_args"]:
        cmd.extend(["--post-tdb-args", str(ctx.params["post_tdb_args"])])
    return cmd
...
def _render_slurm(ctx: RunContext, submit: bool) -> None:
    ctx.slurm_dir.mkdir(parents=True, exist_ok=True)
    groups = ctx.selected_groups
    partition = ctx.params.get("slurm_partition")
    account = ctx.params.get("slurm_account")
    ...
    stage_map = {
        "sniffles": ("pp_sniffles", 24, "12:00:00"),
        "sniffles_filter": ("pp_snifflt", 2, "01:00:00"),
        "kanpig": ("pp_kanpig", 8, "12:00:00"),
        "medaka": ("pp_medaka", 8, "24:00:00"),
        "tdb_create": ("pp_tdbc", 4, "04:00:00"),
        "modkit": ("pp_modkit", 8, "12:00:00"),
    }
    for stage in ctx.stages:
        if stage not in GROUP_SCOPED_STAGES:
            continue
        job_name, cpus, time_limit = stage_map[stage]
        ...
        stage_scripts.append((stage, script_path))

    for stage in ctx.stages:
        if stage in GROUP_SCOPED_STAGES:
            continue
        if stage == "collapse" and len(groups) == 2:
            ...
            _write_slurm_script(
                script_path=script_path,
                job_name="pp_tdbmerge",
                cpus=int(ctx.params["tdb_merge_threads"]),
                time_limit="04:00:00",
                body_lines=body,
                array=None,
                partition=partition,
            ...
            stage_scripts.append((stage, script_path))
        elif stage == "post_tdb" and ctx.params["post_tdb_script"]:
            script_path = ctx.slurm_dir / "post_tdb.sbatch.sh"
            body = [
                f"export PYTHONPATH={shlex.quote(str(REPO_ROOT / 'src'))}:${{PYTHONPATH:-}}",
                shlex.join(_build_recursive_cli(ctx, "post_tdb", ",".join(groups))),
            ]
            _write_slurm_script(
                script_path=script_path,
                job_name="pp_posttdb",
    ...
    submission_plan = {"created_at": _now_utc(), "scripts": [str(path) for _, path in stage_scripts]}
    submitted: dict[str, str] = {}
    if submit and not ctx.dry_run:
        deps: dict[str, str] = {}
        for stage, path in stage_scripts:
            dep_arg = []
            if stage == "sniffles_filter" and "sniffles" in submitted:
                dep_arg = ["--dependency", f"afterok:{submitted['sniffles']}"]
            elif stage == "kanpig" and "sniffles_filter" in submitted:
                dep_arg = ["--dependency", f"afterok:{submitted['sniffles_filter']}"]
            elif stage == "collapse" and "kanpig" in submitted:
                dep_arg = ["--dependency", f"afterok:{submitted['kanpig']}"]
            elif stage == "tdb_create" and "medaka" in submitted:
                dep_arg = ["--dependency", f"afterok:{submitted['medaka']}"]
            elif stage == "tdb_merge" and "tdb_create" in submitted:
                dep_arg = ["--dependency", f"afterok:{submitted['tdb_create']}"]
            elif stage == "post_tdb" and "tdb_merge" in submitted:
                dep_arg = ["--dependency", f"afterok:{submitted['tdb_merge']}"]
            job_id = _run_command_capture(["sbatch", "--parsable", *dep_arg, str(path)])
            submitted[stage] = job_id
            deps[stage] = " ".join(dep_arg)
        submission_plan["submitted_job_ids"] = submitted
        submission_plan["dependencies"] = deps
    _write_json(ctx.slurm_dir / "submission_plan.json", submission_plan)
...
def postprocess_main(args) -> None:
    _setup_logging()
    ...
    if ctx.scheduler == "local":
        ...
        _render_slurm(ctx, submit=bool(getattr(args, "submit", False)))
