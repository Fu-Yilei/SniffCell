...
def parse_args(argv):
    ...
    find_parser.add_argument("-n", "--npy", default=default_npy, help=f"Legacy input .npy matrix for finding cell type DMRs, default={default_npy}")
    find_parser.add_argument("--mdb", default=None, help="Combined MDB atlas directory; when supplied, this is used instead of --npy/--index/--meta")
    find_parser.add_argument("--assay", default="modifiedC", help="MDB assay view to use: modifiedC, 5mC, 5hmC, comma-separated assays, or dual (5mC+5hmC); default=modifiedC")
    find_parser.add_argument("--haplotype", default="combined", help="MDB haplotype view, default=combined")
    find_parser.add_argument("--strand", default="combined", help="MDB strand view, default=combined")
    find_parser.add_argument("--mdb-batch-rows", type=int, default=65536, help="Rows streamed per MDB batch, default=65536")
    find_parser.add_argument("--paired-stability-filter", action="store_true", help="Require paired-donor direction and effect stability before MDB ctDMR merging")
    find_parser.add_argument("--paired-metadata", default=None, help="TSV/CSV with sample_id or id, donor, and cell_type columns for paired filtering")
    find_parser.add_argument("--paired-min-donors", type=int, default=4, help="Minimum complete donor pairs, default=4")
    find_parser.add_argument("--paired-support-effect", type=float, default=0.30, help="Per-donor effect counted as strong support, default=0.30")
    find_parser.add_argument("--paired-min-support", type=int, default=3, help="Minimum donor pairs reaching --paired-support-effect, default=3")
    find_parser.add_argument("--paired-min-effect", type=float, default=0.15, help="Minimum absolute effect in every donor pair, default=0.15")
    ...
    find_parser.add_argument( "--min_rows", type=int, default=1, help="Minimum number of rows (CpG groups in index) for calling DMRs, default=1 (single-row regions allowed)")
