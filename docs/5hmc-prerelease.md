...
An optional PyPI alpha, for example `0.10.0a1`, provides a versioned test
release without replacing the stable version selected by normal pip installs.
This version is a proposal, not a claim that it has been published. After an
alpha is published, testers can explicitly select it:

```bash
pip install 'sniffcell==0.10.0a1'
```

Maintainer procedure:

1. Set `setup.cfg` to `version = 0.10.0a1` and `src/sniffcell/__init__.py` to
   `__version__ = "v0.10.0a1"` on the tested 5hmC branch.
2. Run the test suite, build distributions, and check package metadata.
3. Commit and push the version change, then create and push tag `v0.10.0a1`
