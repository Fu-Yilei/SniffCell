...
def main() -> None:
    ...
    assert int(tr["change_size_bp"]) == 2585
    ...
    assert overlapping_sv, "No SV call overlaps the SH3RF3 repeat interval"
    ...
    assert split_counts == {"Neuron": 164, "Oligodendrocyte": 357}

    assert (output / "anno/SH3RF3_TR_expansion.png").is_file()
    assert (output / "anno/report/index.html").is_file()
    print(
        "PASS: native GRCh38 SH3RF3 example produced a Neuron-only 2,585-bp "
        "TR expansion and an overlapping SV call."
    )
